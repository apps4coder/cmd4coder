...
| 安装 | Install MySQL Server or Client package |
...
- **MEDIUM**: Avoid using passwords in command line (visible in process list)
- 数据库连接强制使用 TLS 加密
...
**Connect as root user with password prompt**:
```bash
mysql -u root -p
```
...
[[mysql|命令详情]] | [[best-practices-MOC|最佳实践总索引]]
