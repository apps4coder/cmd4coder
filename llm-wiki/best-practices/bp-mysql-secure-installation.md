...
# mysql_secure_installation — 生产环境最佳实践
...
- **MEDIUM**: Changes security settings; may lock out users if misconfigured
- 数据库连接强制使用 TLS 加密
...
**指定配置文件运行安全向导**:
```bash
sudo mysql_secure_installation --defaults-file=/etc/mysql/my.cnf
```
