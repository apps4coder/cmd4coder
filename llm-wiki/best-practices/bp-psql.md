...
tags: ["database", "risk-high", "linux", "darwin", "windows"]
created: "2026-07-28"
source_file: "database/postgresql.yaml"
---
...
| 安装 | Install PostgreSQL package |

---

## 生产环境配置

- 调优 `shared_buffers` (25% RAM)、`work_mem`、`effective_cache_size`
- 启用 `pg_stat_statements` 扩展监控查询性能
- 配置流复制 (`streaming replication`) + 自动故障转移
- 定期 `VACUUM ANALYZE` 或配置 autovacuum 参数

## 安全加固

- ⚠️ 此命令风险等级为 **HIGH**，生产环境使用前必须经过变更审批
- **HIGH**: Direct database access; can modify or delete data
- **MEDIUM**: SQL injection risk if using dynamic queries
- 数据库连接强制使用 TLS 加密
...
[[psql|命令详情]] | [[best-practices-MOC|最佳实践总索引]]
